...
abstract class REST_Controller extends CI_Controller {
     ...
     * @return array
     */
    public function dateConvert(array $checkArr)  : array
    {
        $check_columns = [
            'regDate', 'modDate', 'depositDate', 'inspectDate'
        ];
        foreach($checkArr as $key => $val) 
        {
            if (is_array($val))
            {
                $checkArr[$key] = $this->dateConvert($val);
            }
            else
            {
                if (in_array($key, $check_columns)) 
                {
                    if (!is_null( $val)) {
                        $checkArr[$key] = $val == '0000-00-00 00:00:00' ? null : date("Y-m-d\TH:i:s.000\Z", strtotime($val));
                    }
                }
            }
        }
    ...
    {
        //td에 로그전송
        $logs = $this->load->get_vars();

        $logs['logData']['success'] = isset($data['code']) ? $data['code'] : '';
        TdSendLog($logs['logData']);

        /**
         * 서버 시간 TZ 포맷 변경
         * list, info
         */ 
        $resultArr = isset($data['result']) ? (array) $data['result'] : [];
        if (sizeof($resultArr) > 0) 
        {
            $checkArr = []; 
            $keyCheckArr = ['list', 'info', 'data', 'depositList', 'info', 'packageList', 'history'];
            $keyCheck = null;
            foreach($keyCheckArr as $key => $val)
            {
                if (isset($resultArr[$val]))
                {
                    $checkArr = $resultArr[$val];
                    $keyCheck = $val;
                } 
                
                if (sizeof($checkArr) > 0) {
                    $convertData = $this->dateConvert($checkArr);
                    $resultArr[$keyCheck] =  $convertData;
                    $data['result'] = (object) $resultArr;
                }
     ...
     * @return void
     */
    protected function _check_value($data)
    {
